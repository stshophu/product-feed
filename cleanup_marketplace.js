...
async function marketplaceGet(url) {
...
async function getAllMarketplaceProducts() {
  const products = [];
  const seen = new Set();
  let page = 1, totalCount = null, fetched = 0;
  while (true) {
    const url = `${process.env.MARKETPLACE_API_BASE}/api/rest/v1/retailer/products?limit=100&with_count=true&pagination_type=page&page=${page}`;
    const data = await marketplaceGet(url);
    if (totalCount === null) {
      totalCount = data.items_count ?? null;
      console.log(`  Marketplace reports ${totalCount} total listings`);
    }
...
async function safeDeleteFromMarketplace(identifier) {
  const token = await getToken();
...
async function run() {
  ...
  console.log(`\nTotal enabled Marketplace products: ${marketplaceProducts.length}\n`);
    ...
    try {
      ...
      try {
        ...
        if (shopifyErr.response?.status === 404) {
          // Variant no longer exists in Shopify -> true orphan, remove from Marketplace
          process.stdout.write('\n');
          if (DRY_RUN) {
            console.log(`[DRY] Orphan (deleted in Shopify), would delete: ${p.identifier}`);
          } else {
            await withRetry(() => safeDeleteFromMarketplace(p.identifier), p.identifier + " delete");
